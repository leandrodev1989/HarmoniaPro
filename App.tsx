...
// Helper para criar Impulse Response para Reverb
...
const App: React.FC = () => {
  ...
  // Theme State - Carrega do localStorage ou respeita preferência do sistema
  ...
  // Theme Sync - Aplica a classe 'dark' no elemento HTML
  useEffect(() => {
    if (theme === 'dark') {
  ...
  const updateStats = (note: string, isHit: boolean) => {
      setEarStats(prev => {
          ...
          return { ...prev, [note]: { hits: isHit ? current.hits + 1 : current.hits, misses: isHit ? current.misses : current.misses + 1 } };
      });
  };
  ...
  const playChord = useCallback((chord: Chord, forcedInversion?: number) => {
    ...
    if (currentInstrument === 'guitar') {
        ...
        if (voicing.length > 0) {
            ...
            const tuningRefs = [ {n:'E', o:2}, {n:'A', o:2}, {n:'D', o:3}, {n:'G', o:3}, {n:'B', o:3}, {n:'E', o:4} ];
            sortedVoicing.forEach((pos, index) => {
                ...
                setTimeout(() => playTone(getFreq(noteName, octave)), strumDelay);
            });
            return;
        }
    }
    ...
    if (playbackMode === 'block') {
      voicingNotes.forEach((note, i) => setTimeout(() => playTone(getFreq(note.name, note.octave)), i * 10));
    } else {
      const pattern = [...voicingNotes];
      if (pattern.length === 3) pattern.push({ name: pattern[0].name, octave: pattern[0].octave + 1 });
      pattern.forEach((note, i) => {
         const delay = i * (250 * speedMultiplier);
         setTimeout(() => playTone(getFreq(note.name, note.octave), 0.5, 0.2), delay);
      });
    }
  }, [playTone, inversion, playbackMode, bpm, jamMode]); 

  const playNote = (noteName: string, octave: number = 4) => {
    playTone(getFreq(noteName, octave), 0.5);
    if (isExerciseMode) checkNoteInput(noteName, octave);
  };
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 bg-slate-50 dark:bg-gray-900 text-gray-900 dark:text-white flex flex-col font-sans selection:bg-blue-500 selection:text-white pb-20 md:pb-0">
      {/* Header */}
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">Plataforma de Harmonia Funcional</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 bg-gray-100 dark:bg-gray-900 p-1.5 rounded-xl border border-gray-200 dark:border-gray-800">
            ...
            <button
              ...
              className="p-2 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-yellow-400 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
              title="Alternar Tema"
            >
            ...
            {/* Metronome Control */}
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700">
                <button
                   onClick={() => setIsMetronomeOn(!isMetronomeOn)}
                   className={`p-1 rounded-full transition-colors ${isMetronomeOn ? 'text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30' : 'text-gray-500 hover:text-gray-700 dark:hover:text-white'}`}
                   title="Metrônomo"
                >
                    <Timer size={16} />
                </button>
            ...
            {/* MIDI Indicator */}
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border ${midiInputs.length > 0 ? 'bg-green-100 dark:bg-green-900/30 border-green-200 dark:border-green-800 text-green-600 dark:text-green-400' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-500'}`}>
                <Cable size={14} />
            ...
            <div className="relative">
              <select 
                ...
                className="appearance-none bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-900 dark:text-white pl-4 pr-10 py-2 rounded-lg border border-gray-200 dark:border-gray-700 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none cursor-pointer transition-colors font-mono font-bold"
              >
            ...
            <div className="flex bg-white dark:bg-gray-800 rounded-lg p-1 border border-gray-200 dark:border-gray-700 shadow-sm">
               <button 
                 ...
                 className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${scaleType === ScaleType.MAJOR ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}
               >
                 Maior
               </button>
               <button 
                 onClick={() => setScaleType(ScaleType.NATURAL_MINOR)}
                 className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${scaleType === ScaleType.NATURAL_MINOR ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}
               >
                 Menor
               </button>
            </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-6 flex flex-col gap-8">
        ...
        <section className="w-full space-y-4">
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white dark:bg-gray-800/30 p-4 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
            
            {/* Left: View Controls */}
            <div className="flex flex-wrap gap-4 w-full xl:w-auto">
                 <div className="flex bg-gray-100 dark:bg-gray-900 rounded-lg p-1 border border-gray-200 dark:border-gray-700">
                    <button
                        onClick={() => { setInstrument('piano'); setIsExerciseMode(false); }}
                        className={`flex items-center gap-2 px-3 py-1.5 rounded text-xs font-bold transition-all ${instrument === 'piano' ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}
                    >
                        Piano
                    </button>
                    <button
                        onClick={() => { setInstrument('guitar'); setIsExerciseMode(false); }}
                        className={`flex items-center gap-2 px-3 py-1.5 rounded text-xs font-bold transition-all ${instrument === 'guitar' ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}
                    >
                    ...
                    <div className="flex bg-gray-100 dark:bg-gray-900 rounded-lg p-1 border border-gray-200 dark:border-gray-700">
                        {[0, 1, 2].map(i => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded text-xs font-bold transition-all ${inversion === i ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}
                            >
                                {i === 0 ? 'Fund.' : `${i}ª Inv`}
                            </button>
                        ))}
                ...
                {instrument === 'piano' && (
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${showPianoScale ? 'bg-purple-600 text-white border-purple-500 shadow-md' : 'bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700'}`}
                    >
                ...
                {instrument === 'guitar' && (
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${showScale ? 'bg-purple-600 text-white border-purple-500 shadow-md' : 'bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700'}`}
                    >
            ...
            <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto justify-end">
                <button
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-bold transition-all ${jamMode ? 'bg-orange-500/10 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 border-orange-200 dark:border-orange-800' : 'bg-white dark:bg-gray-900 text-gray-500 border-gray-200 dark:border-gray-700'}`}
                ...
                <div className="flex items-center gap-2 bg-white dark:bg-gray-900 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                <div className="flex bg-gray-100 dark:bg-gray-900 rounded-lg p-1 border border-gray-200 dark:border-gray-700">
                    <button
                        ...
                        className={`px-3 py-1.5 rounded text-xs font-bold transition-all ${playbackMode === 'block' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-200 border border-blue-200 dark:border-blue-800' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}
                    >
                        Bloco
                    </button>
                    <button
                        onClick={() => setPlaybackMode('arpeggio')}
                        className={`flex items-center gap-1 px-3 py-1.5 rounded text-xs font-bold transition-all ${playbackMode === 'arpeggio' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-200 border border-blue-200 dark:border-blue-800' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}
                    >
          ...
          {/* Exercise Panel */}
          {isExerciseMode && (
             <div className="w-full bg-white dark:bg-gradient-to-r dark:from-yellow-900/40 dark:to-gray-900 border border-yellow-200 dark:border-yellow-700/50 rounded-xl p-6 relative overflow-hidden animate-fade-in-up shadow-lg">
                ...
                <div className="flex flex-col gap-6 relative z-10">
                   <div className="flex flex-col md:flex-row justify-between gap-4">
                       <div className="flex gap-2">
                           <button 
                             ...
                             className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-colors border ${exerciseType === 'chord' ? 'bg-yellow-500 text-black border-yellow-400' : 'bg-gray-100 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}
                           >
                              Montar Acordes
                           </button>
                           <button 
                             onClick={() => changeExerciseType('ear')}
                             className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-colors border ${exerciseType === 'ear' ? 'bg-yellow-500 text-black border-yellow-400' : 'bg-gray-100 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}
                           >
                              Treino de Ouvido
                           </button>
                       </div>
                       ...
                       {exerciseType === 'ear' && weakestNotes.length > 0 && (
                          <div className="bg-red-50 dark:bg-black/30 border border-red-100 dark:border-white/10 rounded-lg px-3 py-1 flex items-center gap-3">
                              <span className="text-[10px] text-red-600 dark:text-red-300 uppercase font-bold flex items-center gap-1">
                                <AlertCircle size={10} /> Onde Melhorar:
                              </span>
                              <div className="flex gap-1">
                                {weakestNotes.map(([note, stat], i) => (
                                  <span key={i} className="text-xs font-mono font-bold text-red-700 dark:text-white bg-red-100 dark:bg-red-500/20 px-1.5 rounded">
                                    {note}
                                  </span>
                                ))}
                              </div>
                   ...
                   <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                      <div className="text-center md:text-left">
                          <h3 className="text-yellow-600 dark:text-yellow-400 font-bold uppercase tracking-wider text-xs mb-1">
                          ...
                          <div className="mb-2">
                              {exerciseType === 'ear' && (
                                <div className="flex justify-center md:justify-start gap-1 mb-2">
                                    {[0, 1, 2].map(i => (
                                        <Heart key={i} size={16} className={`${i < (3 - attempts) ? 'fill-red-500 text-red-500' : 'text-gray-300 dark:text-gray-600 fill-gray-100 dark:fill-gray-900/50'}`} />
                                    ))}
                                </div>
                              )}
                              <h2 className="text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-3 justify-center md:justify-start">
                                {exerciseType === 'chord' && targetChord ? (
                                    <>Encontre: <span className="text-blue-600 dark:text-yellow-300">{targetChord.symbol}</span></>
                                ) : exerciseType === 'ear' && targetNote ? (
                                    <>Que nota é essa? <span className="text-blue-600 dark:text-yellow-300 bg-blue-50 dark:bg-yellow-900/30 px-3 rounded">?</span></>
                                ) : 'Preparando...'}
                              </h2>
                          </div>
                          <p className="text-gray-500 dark:text-gray-400 text-sm">
                              {exerciseType === 'chord' ? `Toque as ${targetChord?.notes.length} notas deste acorde no piano.` : 'Ouça o som e clique na tecla correspondente. Você tem 3 tentativas.'}
                          </p>
                      </div>

                      <div className="flex gap-4 items-center">
                          {exerciseType === 'ear' && (
                              <button onClick={replayTargetNote} className="flex items-center gap-2 px-4 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-bold text-sm shadow-lg transition-all active:scale-95">
                                  <Volume2 size={18} /> Ouvir
                              </button>
                          )}
                          <div className="bg-gray-100 dark:bg-black/40 rounded-lg p-3 text-center min-w-[100px] border border-gray-200 dark:border-gray-700">
                            <span className="block text-xs text-gray-500 uppercase">Pontos</span>
                            <span className="text-2xl font-mono font-bold text-blue-600 dark:text-green-400">{exerciseScore}</span>
                          </div>
                          <button onClick={() => startExercise(harmonicField, exerciseType)} className="p-3 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300 rounded-full transition-colors" title="Pular">
                            <RefreshCw size={20} />
                          </button>
                      </div>
                   </div>
                ...
                {exerciseFeedback === 'wrong' && (
                 ...
                 {exerciseFeedback === 'gameover' && (
                   <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-200 px-6 py-2 rounded-full text-sm font-bold animate-pulse shadow-lg border border-red-300 dark:border-red-500">
           ...
           <div className="space-y-4">
               <div className="flex flex-wrap justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${isPlaying && !playingProgressionName ? 'bg-red-500/20 text-red-500 border border-red-500/50 hover:bg-red-500/30' : 'bg-blue-600 text-white hover:bg-blue-500 shadow-lg shadow-blue-900/20'}`}
                    >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 md:gap-4">
                {harmonicField.chords.map((chord, idx) => (
                  <ChordCard key={idx} chord={chord} isActive={activeChord?.symbol === chord.symbol} onClick={() => handleChordClick(chord)} onExplain={(e) => handleAiExplain(e, chord)} onAdd={(e) => handleAddToSequence(e, chord)} />
                ))}
              </div>
           </div>
           ...
           {harmonicField.secondaryDominants.length > 0 && (
             <div className="space-y-4 pt-4 border-t border-gray-200 dark:border-gray-800">
                <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 flex items-center gap-2">
                  ...
                  <span className="ml-2 px-2 py-0.5 rounded-full bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 text-[10px] font-mono border border-orange-200 dark:border-orange-500/30">PRO</span>
                </h2>
                <p className="text-sm text-gray-500 dark:text-gray-400">Acordes de empréstimo usados para criar tensão e preparar graus específicos.</p>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-4">
                  {harmonicField.secondaryDominants.map((chord, idx) => (
                    <ChordCard key={`sec-${idx}`} chord={chord} isActive={activeChord?.symbol === chord.symbol} onClick={() => handleChordClick(chord)} onExplain={(e) => handleAiExplain(e, chord)} onAdd={(e) => handleAddToSequence(e, chord)} />
                  ))}
                </div>
             </div>
           )}
        ...
        <section className={`w-full bg-white dark:bg-gray-900/50 border border-dashed border-gray-300 dark:border-gray-700 rounded-xl p-4 md:p-6 space-y-4 relative overflow-hidden shadow-sm ${isExerciseMode ? 'hidden' : ''}`}>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">Clique no (+) dos acordes acima para montar sua música.</p>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => setCustomSequence([])} disabled={customSequence.length === 0} className="px-3 py-1.5 text-xs font-bold text-gray-400 dark:text-gray-500 hover:text-red-500 disabled:opacity-30 transition-colors flex items-center gap-2">
                        <Trash2 size={14} /> Limpar
                    </button>
                    <button onClick={() => playChordSequence(customSequence, 'custom')} disabled={customSequence.length === 0} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all shadow-lg ${isPlaying && playingProgressionName === 'custom' ? 'bg-red-500/20 text-red-500 border border-red-500/50 hover:bg-red-500/30' : 'bg-blue-600 text-white hover:bg-blue-500 disabled:bg-gray-100 dark:disabled:bg-gray-800 disabled:text-gray-400 dark:disabled:text-gray-600 disabled:shadow-none'}`}>
            ...
            <div className="flex gap-2 overflow-x-auto pb-4 min-h-[100px] items-center">
                {customSequence.length === 0 ? (
                    <div className="w-full text-center py-8 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-lg text-gray-400 dark:text-gray-600 text-sm italic">
                    ...
                    customSequence.map((chord, idx) => {
                        ...
                        return (
                            <div key={idx} className="relative group shrink-0 animate-fade-in-up" style={{ animationDelay: `${idx * 0.05}s` }}>
                                <div onClick={() => handleChordClick(chord)} className={`flex flex-col items-center justify-center w-24 h-24 rounded-xl border-2 transition-all cursor-pointer ${isActiveInSeq ? 'bg-blue-100 dark:bg-blue-500/20 border-blue-600 dark:border-blue-500 shadow-md scale-105' : 'bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500'}`}>
                                    <span className="text-xs text-gray-500 font-mono mb-1">{chord.roman}</span>
                                    <span className="text-xl font-bold text-gray-900 dark:text-white">{chord.symbol}</span>
                                </div>
                                <button onClick={() => handleRemoveFromSequence(idx)} className="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white rounded-full p-1 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity scale-75 group-hover:scale-100">
                                    <X size={12} strokeWidth={3} />
                                </button>
                                {idx < customSequence.length - 1 && <div className="absolute top-1/2 -right-4 transform -translate-y-1/2 text-gray-300 dark:text-gray-700 z-0"><ChevronsRight size={16} /></div>}
                            </div>
                        )
                    })
                )}
        ...
        {/* Common Progressions */}
        <section className={`w-full space-y-4 pb-20 border-t border-gray-200 dark:border-gray-800 pt-8 ${isExerciseMode ? 'hidden' : ''}`}>
          <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {progressions.map((prog, idx) => {
                ...
                return (
                  <div key={idx} className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl p-4 flex flex-col gap-3 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors group shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                          <h3 className="font-bold text-gray-900 dark:text-gray-100 group-hover:text-blue-600 dark:group-hover:text-blue-300 transition-colors">{prog.name}</h3>
                          <p className="text-xs text-gray-500 dark:text-gray-400">{prog.description}</p>
                        </div>
                        <button onClick={() => playChordSequence(progChords, prog.name)} className={`p-2 rounded-full transition-colors ${isThisPlaying ? 'bg-red-500/20 text-red-500 animate-pulse' : 'bg-blue-600/10 dark:bg-blue-600/20 text-blue-600 dark:text-blue-400 hover:bg-blue-600 hover:text-white'}`}>
                      ...
                      {prog.degrees.map((degree, i) => {
                        ...
                        return (
                          <div key={i} className="flex items-center">
                            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm font-mono transition-all duration-300 ${isActiveInProg ? 'bg-yellow-100 dark:bg-yellow-500/20 border-yellow-600 dark:border-yellow-500 text-yellow-700 dark:text-yellow-200 scale-105 shadow-md' : 'bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300'}`}>
                                <span className="opacity-50 text-xs">{degree}</span>
                                <span className="font-bold">{c?.symbol || '?'}</span>
                            </div>
                            {i < prog.degrees.length - 1 && <ChevronsRight size={12} className="mx-1 text-gray-300 dark:text-gray-600" />}
                          </div>
                        );
                      })}
                    </div>
      ...
      <footer className="bg-white dark:bg-gray-950 border-t border-gray-200 dark:border-gray-800 py-6 text-center text-gray-500 text-sm">
      ...
      <MusicTeacherChat isOpen={isChatOpen} onToggle={() => setIsChatOpen(!isChatOpen)} messages={chatMessages} onSendMessage={handleSendMessage} isLoading={isChatLoading} onClearHistory={handleClearHistory} />
    </div>
  );
};
